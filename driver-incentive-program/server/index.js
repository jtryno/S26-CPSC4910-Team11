...
app.get('/api/dashboard/reviews/:driverUserId', async(req, res) => {
    ...
    try {
        ...
        console.error('Erorr fetching driver\'s reviews', error);
...
app.post('/api/driver-reviews', async (req, res) => {
    ...
    try {
        ...
        //inserts if new updates if exists already
        await pool.query(
            `INSERT INTO sponsor_driver_reviews
                (sponsor_user_id, driver_user_id, rating, review_text)
             VALUES (?, ?, ?, ?)
             ON DUPLICATE KEY UPDATE
                rating = VALUES(rating),
                review_text = VALUES(review_text),
